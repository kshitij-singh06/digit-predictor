...
# ==============================
# 0. SESSION STATE INITIALIZATION (NEW)
# ==============================
# Initialize a counter for the canvas key. Changing this key forces the canvas to reset.
...
def load_digit_model(model_path):
    """Loads model only once across all users and reruns using Streamlit's cache."""
    try:
        # We use compile=False as we only need the weights for inference
...
def preprocess_canvas_image(canvas_image_rgba):
    """
    Convert canvas RGBA image to MNIST-like 28x28 grayscale normalized image.
    This function is crucial and must match the MNIST training pipeline.
    """
    # 1. Convert to grayscale (Luminance)
    img = Image.fromarray(canvas_image_rgba.astype('uint8'), mode='RGBA').convert('L')
    arr = np.array(img)

    # Identify drawn (white) pixels — ignore black background
    # Since st_canvas draws white on black, we look for high values
    nonzero = np.where(arr > 10)
    if nonzero[0].size == 0:
        # If nothing drawn, return blank 28x28
        return np.zeros((1, 28, 28, 1), dtype=np.float32)

    # 2. Crop bounding box around digit
    ...
    # 3. Resize to fit inside 20x20 box (preserving aspect ratio)
    h, w = cropped.shape
    target_size = 20
    if h > w:
        new_h = target_size
    ...
    # Use Image.BILINEAR for high quality resizing
    resized = Image.fromarray(cropped).resize((new_w, new_h), Image.BILINEAR)

    # 4. Create new 28x28 black image and paste resized digit centered
    ...
    # We DO NOT invert the image (255.0 - final_arr) because the canvas
    # already provides white-on-black, matching the MNIST data.
    # The line was removed.
    # ---------------------------------

    # 5. Center the mass (shifting the digit to the center of gravity)
...
canvas_result = st_canvas(
    ...
    # --- KEY MANAGEMENT FIX ---
    key=f"canvas_{st.session_state['canvas_key']}", # Dynamic key forces reset
    # --------------------------
...
if clear_btn:
    # 1. Increment the key to guarantee a new canvas instance is rendered
    st.session_state['canvas_key'] += 1
    # 2. Force a full page reset
    st.rerun()
...
if predict_btn:
    if canvas_result.image_data is not None:
        ...
        if np.sum(img_input) == 0:
            ...
            # Show processed 28x28 image (for clarity)
            st.write("### Processed Input (28x28)")
            # Scale the image for better visualization (0-1 values -> 0-255)
